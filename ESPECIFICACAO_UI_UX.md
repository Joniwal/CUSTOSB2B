...
- Filtros agrupados no topo do painel: busca, status, tipo, tecnologia e limpeza.
- Tabela semântica com as colunas obrigatórias `ID`, `Tipo de Atividade`, `Status`, `Situação` e `Ações`.
- Botão de calculadora antes do ID e coluna DRAFT ao final. DRAFT também está presente nos formulários de inclusão e edição.
- Botão Exportar Excel no cabeçalho baixa a aba completa, com estilos e tabela, sem DRAFT e sem modificar a base.
