(() => {
  ...
  const listState = {
    ...
    q: "", status: "", type: "", sort: "id", direction: "asc", selectedKey: null,
  ...
  async function loadActivities({ announce = false } = {}) {
    const params = new URLSearchParams({
      q: listState.q, status: listState.status, type: listState.type,
    ...
    try {
      ...
      setConnection(false);
      document.querySelector("#activities-body").innerHTML = `<tr><td colspan="6"><div class="empty-state">${iconEmpty}<p>${escapeHtml(error.message)}</p></div></td></tr>`;
  ...
  function renderActivities() {
    ...
    if (!listState.items.length) {
      body.innerHTML = `<tr><td colspan="6"><div class="empty-state">${iconEmpty}<p>Nenhuma atividade corresponde aos filtros.</p></div></td></tr>`;
      return;
    }
  ...
  function setupListControls() {
    ...
    document.querySelector("#clear-filters").addEventListener("click", () => {
      listState.q = ""; listState.status = ""; listState.type = ""; listState.page = 1;
      search.value = "";
  ...
  const activityFields = [
    ...
    "custo_mo", "custo_mat", "custo_evitado", "qtde_tecnicos", "qtde_dias",
  ];
