...
def normalize_activity(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    for field in FIELD_NAMES:
        ...
        elif field in {"atualizado_em", "data"}:
...
def aggregate_by(activities: list[dict[str, Any]], field: str) -> list[dict[str, Any]]:
    ...
    for item in activities:
        ...
        bucket = grouped.setdefault(label, {"label": label, "quantity": 0, "cost": 0.0, "gap": 0.0})
...
def _sample_fallback_enabled() -> bool:
    return os.getenv("EXCEL_FALLBACK_SAMPLE", "true").strip().casefold() in {"1", "true", "yes", "sim"}
...
def discover_excel_file(filename: str, *, base_dir: Path) -> Path:
    """Find one workbook by name in locally synchronized OneDrive folders."""
    filename = filename.strip()
    if (
        not filename
        or Path(filename).name != filename
        or filename.startswith("~$")
        or Path(filename).suffix.casefold() not in {".xlsx", ".xlsm"}
    ):
        raise RepositoryError("EXCEL_FILENAME deve conter apenas o nome de um arquivo .xlsx ou .xlsm, sem pastas.")
        ...
        try:
            for current, directories, files in os.walk(root, topdown=True, followlinks=False):
                ...
                for actual_name in files:
                    if actual_name.startswith("~$") or actual_name.casefold() != target_name:
                        continue
    ...
    if len(ordered) > 1:
        ...
        raise RepositoryError(
            f"Foram encontradas {len(ordered)} cópias de '{filename}'. "
            f"Configure EXCEL_SEARCH_ROOTS ou EXCEL_PATH para indicar a correta:\n{locations}"
        )

    sample = (base_dir / "data" / filename).resolve()
    if _sample_fallback_enabled() and sample.is_file():
    ...
    raise RepositoryError(
        f"O arquivo '{filename}' não foi encontrado nas pastas OneDrive/SharePoint sincronizadas ({searched})."
    )
...
class LocalExcelRepository:
    ...
    def _locate_file(self) -> Path:
        ...
        if explicit:
            ...
            return path
        return discover_excel_file(self.file_name, base_dir=self.base_dir)
    ...
    def _locate_reference_workbook(self, kind: str) -> Path:
        definitions = {
            "services": ("SERVICES", "SERVICOS.xlsx", "serviços"),
            "materials": ("MATERIALS", "MATERIAL.xlsx", "materiais"),
        }
        if kind not in definitions:
            raise ValueError("Tipo de planilha de referência inválido.")
        prefix, default_name, label = definitions[kind]
        ...
        if (
            ...
            raise RepositoryError(
                f"{prefix}_EXCEL_FILENAME deve conter apenas o nome de um arquivo .xlsx ou .xlsm."
            )
            ...
            try:
                match = next(
                    (
                        ...
                        if child.is_file() and child.name.casefold() == filename.casefold()
                    ),
                    None,
                )
        ...
        try:
            return discover_excel_file(filename, base_dir=self.base_dir)
        except RepositoryError as exc:
            raise RepositoryError(
                f"A planilha de {label} '{filename}' não foi encontrada. "
